...
```{r exercise1-check}
grade_result(
  pass_if(~ identical(.result, summary(aov( breaks~wool, data = warpbreaks))), "Good Job!"),
  fail_if(~ TRUE)
)
```
...
```{r exercise4-check}
grade_result(
  pass_if(~ identical(.result, summary(aov( breaks~tension * wool, data=warpbreaks))), "Good Job!"),
  fail_if(~ TRUE)
)
```
