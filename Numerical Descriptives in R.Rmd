...
In the examples for this tutorial, we will use the \texttt{iris} dataset provided in \texttt{MASS} package in R. 
...
We can redefine the names of the axes, or even the name of the plot using some specific arguments like `xlab`, `ylab` and `main`. But the names should be provided as a `string` i.e. to be careful to put "" around the names! For example,
...
Consider the `mtcars` dataset in package `MASS`.
